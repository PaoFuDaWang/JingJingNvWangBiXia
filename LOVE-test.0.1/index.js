...
S.ShapeBuilder = (function () {
  var gap = 8, // 手机端增大粒子间距，减少粒子数量
      shapeCanvas = document.createElement('canvas'),
      shapeContext = shapeCanvas.getContext('2d'),
      fontSize = 400, // 手机端减小基础字体大小
  ...
  return {
    ...
    letter: function (l) {
      ...
      var chineseFactor = /[\u4e00-\u9fa5]/.test(l) ? 1.2 : 1; 
      s = Math.min(fontSize,
                  (shapeCanvas.width / shapeContext.measureText(l).width) * 0.7 * fontSize,
                  (shapeCanvas.height / fontSize) * (isNumber(l) ? 1 : 0.6) * fontSize * chineseFactor);
      setFontSize(s);
